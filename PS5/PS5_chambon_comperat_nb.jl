...
# ╔═╡ befc687a-af3e-11ef-3393-0176e7297f12
begin
...
VFI = function(r, agrid, sgrid, V0, prob, par)
    ...
    for iter in 1:par.maxits
        for is in 1:Ns  # Loop over exogenous states (shocks)
            for ia in 1:Na  # Loop over asset levels
                for ia_p in 1:Na
                    EV = sum(prob[is, :] .* Vguess[:, ia_p])
                    tv[ia_p] = U[is, ia, ia_p] + par.beta * EV
                end
                   
                Vnew[is, ia] = maximum(tv)  
                policy_a_index[is, ia] = argmax(tv)  
            end
        end
        ...
        Vguess .= Vnew  # Update the value function guess for the next iteration
    end
...
aiyagari = function(r, par, agrid, sgrid, prob, Vguess)
    ...
    ### TO FILL ####### 
	### Condition to ensure only choices consistent with policy are considered?
    
	for is in 1:Ns
        for ia in 1:Na
            for is_p in 1:Ns
                for ia_p in 1:Na
                Q[is,ia,is_p,ia_p] = prob[is,is_p] * (policy_a_index[is,ia] == ia_p ? 1.0 : 0.0)
                end
            end
        end
    end
    ...
    agg_a = sum(dist[:] .* repeat(agrid, Ns))
...
# ╔═╡ 8e9ae812-5727-4397-850b-9b0865fffb92
md"
# Problem Set 5

## Lionel Chambon & Etienne Compérat
"

# ╔═╡ 6bb15db1-83ac-4bb4-9aac-ea24c04161fc
md"
### Computation
"

# ╔═╡ c1ab436d-9c13-4d34-9171-4ed97418f99f
md"
We break the code here to test if it works before calculating the equilibrium interest rate.
"

# ╔═╡ afa81a64-4d22-429d-9f32-a9be7d361552
begin
    ...
    iter = 0
    while abs(r_max - r_min) > par.tol && iter < par.maxits
        iter += 1
        r_mid = (r_min + r_max) / 2

        # Capital supply        
        k_s = aiyagari(r_mid, par, agrid, sgrid, prob, Vguess)

        # Capital demand
        k_d = demand_k(r_mid, par)

        # Update bounds
        if k_s > k_d
            r_max = r_mid  # Too much supply, reduce rate
        else
            r_min = r_mid  # Too little supply, increase rate
        end
    end
    ...
    println("Equilibrium interest rate found: $((r_min + r_max) / 2) after $iter iterations")
    return (r_min + r_max) / 2
		
	end

# Labor parameter
agg_target = 1 # Normalize labor to 1?

# Initial bounds for interest rate
r_min, r_max = -par.delta, (1/par.beta)-1

# Find equilibrium rate
r_eq = find_eq_r(r_min, r_max, par, agrid, sgrid, prob, Vguess, agg_target)

println("Equilibrium interest rate: ", r_eq)
	
end

# ╔═╡ 567d3363-558d-4a55-abdd-e13746748151
md"
After runing the code several times, we find a very low interest rate of around
1%. Perhaps we accidentally modelled the pre-pandemic Eurozone...
"
...
begin
    ...
    # Function to compute and plot equilibrium
    function plot_eq(r_min, r_max, par, agrid, sgrid, prob, Vguess)
        
        r_vec = collect(range(r_min, r_max, length=100))  

        # Compute capital supply and demand vectors
        k_s_vec = [aiyagari(r, par, agrid, sgrid, prob, Vguess) for r in r_vec]
        k_d_vec = [demand_k(r, par) for r in r_vec]

        # Plot the supply and demand curves
        plot(
            k_s_vec, r_vec,
            label="Capital Supply (Aiyagari)",
            color=:blue,
            lw=2,
            xlabel="Capital (K)",
            ylabel="Interest Rate (r)",
            title="Capital Market Equilibrium",
            xlims=(minimum(k_s_vec) * 0.9, maximum(k_s_vec) * 1.1),  
            ylims=(minimum(r_vec) * 0.9, maximum(r_vec) * 1.1)       
        )

        plot!(
            k_d_vec, r_vec,
            label="Capital Demand (FOC)",
            color=:red,
            lw=2
        )
	end 

    plot_eq(r_min, r_max, par, agrid, sgrid, prob, Vguess)
...
begin
    ...
    function compute_gini(agg_a, dist)
        ...
        # Calculate the total sum of assets
        total_assets = sum(sorted_assets)
        
        # Compute the Lorenz curve (cumulative share of total assets)
        lorenz_curve = cumulative_assets / total_assets
        
        # Add the starting point (0,0) for the Lorenz curve
        lorenz_curve = vcat([0.0], lorenz_curve)
        
        # Create an array for the x-axis (cumulative population share)
        cumulative_population = LinRange(0, 1, length=length(lorenz_curve))
        
        # Compute the area between the Lorenz curve and the line of perfect equality 
		
        area_under_curve = trapz(cumulative_population, lorenz_curve)
        
        # Gini is calculated as: G = 1 - 2 * area under the Lorenz curve
    ...
    function get_gini_from_distribution(agrid, dist)
 
        assets = agrid .* dist

        gini = compute_gini(assets, dist)
...
manifest_format = "2.0"
project_hash = "f57e0138dea7cd98887ca692686b7c51b98dcc50"
